...
class CaseDisciplinary(models.Model):
    ...
    case_do_id = fields.Many2one('matter.matter',string='Case Number',required=False)
    ...
    misconduct_by = fields.Selection([('client','Client'),('accuse','Claimant / Plaintiff')],string="Misconduct By",track_visibility='onchange',required=False)
    client_id = fields.Many2one('client.client',string="Client")
    accuse_id = fields.Many2one('accuse.details',string="Claimant / Plaintiff")
    do_name = fields.Char(string='Name',track_visibility='onchange',required=False)
